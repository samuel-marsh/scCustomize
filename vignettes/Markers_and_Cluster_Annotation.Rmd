...
scCustomize has several helper functions to assist with identication of marker genes and annotation of clusters.
Load packages & Data
```{r init}
# Load Packages
library(tidyverse)
...
```{r}
# Read in precomputed data.frame results
...
## Pairing `Extract_Top_Markers` with `Iterate_Custom_FeaturePlots`
The advtanage of exporting a named vector from `Extract_Top_Markers` is that `Iterate_Custom_FeaturePlots` can detect the names and either add those to the title of the plot (if `single_pdf = TRUE`) or the file names if `FALSE`.
```{r eval=FALSE}
Iterate_Custom_FeaturePlots(seurat_object = pbmc, gene_list = top_5_presto, single_pdf = T)
```
...
* One entry containing vector of cell subtypes (if present).
```{r echo=FALSE, fig.align='center', fig.cap="*Example output of `Pull_Cluster_Annotation*", fig.show='hold'}
knitr::include_graphics("../docs/articles/assets/annotation_info.png")
```
