...
### Split within groups
User can also supply a `split.by` variable to quantify expression within group split by meta data variable
```{r eval=FALSE}
percent_express <- Percent_Expressing(seurat_object = pbmc, features = c("CD4", "CD8A"), split.by = "group")
```
...
p2 <- Plot_Median_Genes(seurat_object = pbmc, group.by = "group")

p3 <- Plot_Median_Other(seurat_object = pbmc, median_var = "module_score1", group.by = "group")
