...
If adding score to human data both the union score and "microglia" (post-mortem) score will be added to be meta.data slot.  When species is mouse only the union score will be added.  

```{r}
pbmc <- exAM_Scoring(seurat_object = pbmc, species = "human")
```

`exAM_Scoring` also supports use of ensembl IDs by specifying optional parameters.  

```{r eval=FALSE}
pbmc <- exAM_Scoring(seurat_object = pbmc, species = "human", ensembl_ids = TRUE)
```
