...
**NOTE: This function is currently only available for human and mouse as those are only species with MALAT1 annotated**  
...
```{r message=TRUE}
hcabm <- Add_MALAT1_Threshold(object = hcabm, species = "human", sample_col = "orig.ident")
```
...
```{r}
VlnPlot_scCustom(seurat_object = hcabm, features = "nFeature_RNA", split.by = "MALAT1_Threshold")
FeatureScatter_scCustom(seurat_object = hcabm, feature1 = "nCount_RNA", feature2 = "nFeature_RNA", split.by = "MALAT1_Threshold", pt.size = 1)
```
...
```{r eval=FALSE}
hcabm <- Add_MALAT1_Threshold(object = hcabm, species = "human", sample_col = "orig.ident", save_plots = TRUE, save_plot_path = "plots/QC_Plots/", save_plot_name = "MALAT1_Threshold_Plots")
```

Here is eample of plots for one of the samples in the hcabm dataset.
