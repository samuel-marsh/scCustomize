...
```{r include=FALSE}
raw_metrics <- qs_read("assets/seq_metrics_dataframe_RAW.qs2")
meta_metrics <- qs_read("assets/seq_metrics_dataframe_meta.qs2")
```
...
Seq_QC_Plot_Basic_Combined(metrics_dataframe = metrics_final, group.by = "Full_Batch")
```

### Alignment Metrics Summary
A set of alignment-based metrics can be plotted using `Seq_QC_Plot_Alignment_Combined`
```{r, fig.height=9, fig.width=13}
Seq_QC_Plot_Alignment_Combined(metrics_dataframe = metrics_final, group.by = "Full_Batch")
```
...
```{r, fig.height=9, fig.width=13}
Seq_QC_Plot_Basic_Combined(metrics_dataframe = metrics_final, group.by = "Full_Batch", significance = F)
```
