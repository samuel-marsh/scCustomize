...
pbmc <- Add_Mito_Ribo(seurat_object = pbmc, species = "human")
```
...
```{r echo=FALSE}
accepted_names <- Add_Mito_Ribo(list_species_names = TRUE)
...
pbmc <- Add_Mito_Ribo(seurat_object = pbmc, species = "other", mito_pattern = "regexp_pattern", ribo_pattern = "regexp_pattern")

# Using feature name lists
mito_gene_list <- c("gene1", "gene2", "etc")
ribo_gene_list <- c("gene1", "gene2", "etc")
pbmc <- Add_Mito_Ribo(seurat_object = pbmc, species = "other", mito_features = mito_gene_list, ribo_features = ribo_gene_list)

# Using combination of gene lists and gene name patterns
pbmc <- Add_Mito_Ribo(seurat_object = pbmc, species = "Human", mito_features = mito_gene_list, ribo_pattern = "regexp_pattern")
```

### Warning Messages
The added benefit of `Add_Mito_Ribo` & `Add_Mito_Ribo` is that they will return informative warnings if no Mitochondrial or Ribosomal features are found using the current species, features, or pattern specification.
```{r message=TRUE, warning=TRUE, error=TRUE}
# For demonstration purposes we can set `species = mouse` for this object of human cells
pbmc <- Add_Mito_Ribo(seurat_object = pbmc, species = "mouse")
```
...
pbmc <- Add_Mito_Ribo(seurat_object = pbmc, species = "other", mito_pattern = "^MT-", ribo_pattern = "BAD_PATTERN")
```

`Add_Mito_Ribo` and `Add_Mito_Ribo()` will also return warnings if columns are already present in `@meta.data` slot and prompt you to provide override if you want to run the function.
```{r include=FALSE}
pbmc <- pbmc3k.SeuratData::pbmc3k
pbmc <- UpdateSeuratObject(object = pbmc)
pbmc <- Add_Mito_Ribo(seurat_object = pbmc, species = "human")
...
scCustomize also contains identical function for use with LIGER objects.  `Add_Mito_Ribo` contains equivalent parameters and capabilities as `Add_Mito_Ribo`
```{r eval=FALSE}
liger_obj <- Add_Mito_Ribo(seurat_object = liger_obj, species = "human")
```
...
```{r eval = FALSE}
# These defaults can be run just by providing accepted species name
pbmc <- Add_Cell_Complexity(seurat_object = pbmc)
```
*NOTE: There is analogous function for LIGER objects (see: `Add_Cell_Complexity()`).*
