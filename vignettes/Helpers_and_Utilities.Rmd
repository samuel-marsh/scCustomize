...
scCustomize contains easy wrapper function to automatically add both Mitochondrial and Ribosomal percentages to meta.data slot.  If you are using mouse, human, marmoset, zebrafish, rat, or drosophila data all you need to do is specify the `species` parameter.
```{r eval=FALSE}
...
#### Non-default species
However custom prefixes can be used for non-human/mouse/marmoset/rat/zebrafish/drosophila species with different annotations. Simply specify `species = other` and supply feature lists or regex patterns for your species of interest. 
